...
int scheduler_policy = 0;  // SCHED_RR = 0
int time_quantum = 10;            // Default: 10 ticks
...
// --- AegisOS: MLFQ function prototypes ---
//static struct proc* mlfq_find_highest(void);
static void mlfq_boost(void);
static void mlfq_update_priority(struct proc *p);
...
// --- AegisOS: Initialize statistics counters ---
  p->cpu_time = 0;
  p->wait_time = 0;
...
// --- AegisOS: Initialize MLFQ fields ---
...
scheduler(void)
{
  ...
  for (;;) {
    ...
    // --- AegisOS: Choose scheduler based on policy ---

    if (scheduler_policy == SCHED_MLFQ) {
    // Find the highest priority process (no locks held across swtch)
    struct proc *best = 0;
    int best_level = 3;  // MLFQ_LOW + 1
    
    // First pass: find the best process
    for (p = proc; p < &proc[NPROC]; p++) {
        acquire(&p->lock);
    ...
    if (p) {
        // Acquire lock for the chosen process
        acquire(&p->lock);
        
        // Double-check it's still runnable (could have changed)
        if (p->state == RUNNABLE) {
            now = ticks;
            p->last_run = now;
            if (p->ready_time > 0) {
      ...
      for (p = proc; p < &proc[NPROC]; p++) {
        ...
        if (p->state == RUNNABLE) {
          ...
          found = 1;
          release(&p->lock);
          break;  // RR picks one process per round
        }
        release(&p->lock);
      }
    }
...
// --- AegisOS: MLFQ Helper Functions ---

// Find the highest-priority process in MLFQ
/*
static struct proc*
mlfq_find_highest(void)
{
  struct proc *p;
  struct proc *highest = 0;
  int best_level = MLFQ_LOW + 1;

  for (p = proc; p < &proc[NPROC]; p++) {
    acquire(&p->lock);
    if (p->state == RUNNABLE && p->mlfq_level < best_level) {
      if (highest)
        release(&highest->lock);   // release previous best before replacing
      best_level = p->mlfq_level;
      highest = p;
      continue; // keep this p's lock held for the caller
    }
    release(&p->lock);
  }
  return highest; // caller gets it with lock held, or 0 with nothing held
}
*/   
